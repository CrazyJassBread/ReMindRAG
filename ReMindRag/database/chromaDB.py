...
class ChromaDBManager:
    ...
    def add_chunk(self, chunk):
        ...
        self.connection_collection.add(
            ids=[f"{anchor_id}_{chunk_id}"],
            embeddings = [np.zeros(self.hidden_size, dtype=np.float32)],
            metadatas=[self._edge_metadata({"entity_id": anchor_id, "chunk_id": chunk_id})],
            documents = [chunk["title"]]
        )
...
# new
    ...
    def _edge_metadata(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        metadata = dict(metadata)
        metadata.setdefault("success_count", 0)
        metadata.setdefault("fail_count", 0)
        metadata["confidence"] = self._edge_confidence(metadata["success_count"], metadata["fail_count"])
        return metadata

    def _edge_confidence(self, success_count: int, fail_count: int) -> float:
        return (int(success_count) + 1) / (int(success_count) + int(fail_count) + 2)

    def _get_edge_collection(self, edge_type: str):
        if edge_type == "relation":
            return self.relation_collection
        return self.connection_collection

    def _get_edge_memory(self, edge_type: str, edge_id: str):
        collection = self._get_edge_collection(edge_type)
        edge_data = collection.get(ids=[edge_id], include=["embeddings", "metadatas"])
        if not edge_data["ids"]:
            self.logger.warning(f"Edge {edge_id} not found while updating memory.")
            return None, None, None, 0.5

        metadata = self._edge_metadata(edge_data["metadatas"][0] or {})
        confidence = self._edge_confidence(metadata["success_count"], metadata["fail_count"])
        embedding = np.array(edge_data["embeddings"][0])
        return collection, embedding, metadata, confidence

    def _bump_edge_memory_stats(
        self,
    ...
    def enhance_edge_weight(self, query:str, path_list:List[Tuple[str,str]]):
        ...
        query_embedding = query_embedding / np.linalg.norm(query_embedding)
        for path in path_list:
            collection, embedding, metadata, confidence = self._get_edge_memory(path[0], path[1])
            if collection is None:
                continue

            self.logger.info(f"Enhance Path {path[1]} with confidence {confidence:.4f}.")
            new_embedding = embedding + confidence * query_embedding * edge_weight_coefficient(np.linalg.norm(embedding))
            metadata = self._bump_edge_memory_stats(metadata, success_delta=1)
            collection.update(
                ids=[path[1]],
                embeddings=[new_embedding],
                metadatas=[metadata]
    ...
    def punish_edge_weight(self, query:str, path_list:List[Tuple[str,str]]):
        ...
        for path in path_list:
            ...
            self.logger.info(f"Punish Path {path[1]} with confidence {confidence:.4f}.")
            metadata = self._bump_edge_memory_stats(metadata, fail_delta=1)
            query_direction_vector = np.dot(query_embedding, embedding)
            if (np.dot(query_direction_vector, query_embedding) >= 0).all():
                punish_coefficient = math.fabs(edge_weight_coefficient(np.linalg.norm(query_direction_vector)))
                punish_weight = 1 - confidence
                new_embedding = embedding - punish_weight * max(punish_coefficient, np.linalg.norm(query_direction_vector)) * query_direction_vector
                collection.update(
                    ids=[path[1]],
                    embeddings=[new_embedding],
                    metadatas=[metadata]
